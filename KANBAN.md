# KANBAN — Last updated: 2026-04-01 11:00 PM PST (Q2 Day 1)

## 🔴 URGENT / BLOCKED
- [ ] **Heather (Amazon customer) — 17 DAYS** — seller health emergency. Reply IMMEDIATELY. 5 min fix. Draft ready in comms package.
- [ ] **Insurance audit — 60 DAYS overdue** — policy GW7487, Ashlin Hadden. Sales data ready ($637,650.12). Ev must fill PDF and submit to audits@veracityins.com. Non-renewal risk is real.
- [ ] **ShipBob UROs — 53 days, $954+ accrued** ($18/day) — 2 shipments On-Hold Receiving. Harsh Khanna (ShipBob) escalated to US West Hub ops 3/26. SLA likely expired. Needs Ev to check dashboard.
- [ ] **Shopify API dead (39 days)** — both tokens return auth errors. Blocks inventory, orders, restock. 15 min fix: regen tokens in Partners dashboard.
- [ ] **Amazon CA suppressed (15 days, $225+ lost)** — NARF/SDS compliance. All CA listings down. Submit WHMIS SDS appeal. ~55 min.

## 🟡 NEEDS EV / WAITING
- [ ] **Donaldson $2,340 — SHIP NOW** — Rachael paid, address confirmed (1801 W Vine St, Harrisonville MO 64701). Ship before Good Friday Apr 3.
- [ ] **Dynasty Global (Eli)** — legit dealer inquiry 3/26, works with WD-40/3-in-1. Ev decision needed. 6 days waiting.
- [ ] **Teikametrics $149 dispute** — INV33246 charged 3/30 despite 2/24 cancellation. Email ar@teikametrics.com. Draft ready.
- [ ] **Walmart unshipped order** — past ship date, auto-cancellation warning received
- [ ] **Ethan Miller replacement** — order #5786, 4× Thick 16oz, not created at ShipBob
- [ ] **Josh Mintz sample** — 8oz Medium to 5008 2B Magazine St, New Orleans LA 70115. 9 days overdue, cooling.
- [ ] **Christopher Robin Webber** — B2B inquiry, 9 days old, going cold. Draft reply ready (~$300/mo).
- [ ] **KMS LLC (Brittni)** — wholesale distributor, 13 days silence. Draft follow-up ready (~$500/mo).
- [ ] **Jason F return** — decision pending
- [ ] **B2B Batch 1 outreach** — EDCO, Crown Awards, PersonalizationMall. Strategy docs + templates ready. Awaiting Ev go.
- [ ] **Vercel billing** — $53.31 card retry 3/20, site still up. Confirm payment.

## 🔵 IN PROGRESS
- [ ] **Amazon PPC — 11 active campaigns** — $773/day spend (3/31), $1,867 sales, 2.42× ROAS, 41.4% ACoS. Above 7-day average. Recovery from negative keyword collapse confirmed.
- [ ] **A3 Partners (Caroline Silvestro)** — PO 28 shipping, UPS ETA Apr 3. Invoice #26-03263 $3,312.37 due 4/25. Caps next week.
- [ ] **Noveon Magnetics** — $4,140 quote, 18 days waiting. Likely dead. Draft Hail Mary follow-up ready with trial order option.
- [ ] **Ads daily pull** — bug fixed 3/31 (timeout 600→1200s, retry logic, DATA QUALITY WARNING banner). Data flowing again.
- [ ] **Designcoffers** — pump accelerator label revisions in progress (Fiverr)

## ✅ DONE (Q1 Close / Recent)
- [x] **Donaldson $2,340** — PAID via CC. Address confirmed 3/31.
- [x] **Garcor Supply** — PAID $4,806.00 via PayPal 3/25
- [x] **R&R Fabrications** — PAID $1,289.70 via PayPal 3/25
- [x] **Ron Southern** — PAID invoice #1001-0237 via PayPal 3/27
- [x] **DMS Packaging** — inbound shipment 26SAL03024 received at Elk Grove Village 3/27
- [x] **PPC negative keyword collapse** — 79 negatives removed 3/20, optimizer patched. Recovery confirmed.
- [x] **Ads daily pull bug** — timeout fix + retry logic deployed 3/31
- [x] **Amazon Ads Reporting API** — recovered after 9 days (3/23-3/31)
- [x] **Jerry Adamson** — military discount refund handled, closed
- [x] **Q1 Final Scorecard** — built 3/31 nightly
- [x] **7 draft communications** — prepared 3/31, covering $7,282+ revenue impact
- [x] **B2B strategy review** — re-read whale-outreach + sales playbook 3/31

## 📋 BACKLOG
- [ ] Wire flywheel data into MoneySamurai PPC Diagnostic frontend
- [ ] Fix $0-spend campaigns: Thick CA Glue ($16.36 spent, 0 sales on 3/31) — bid/targeting issue
- [ ] Competitor ASIN identification
- [ ] Back-in-stock notification app (Shopify)
- [ ] Refund policy still links to gravixadhesives.com
- [ ] Google Search Console sitemap submission
- [ ] Opinew review import verification (1,183 reviews)
- [ ] Teikametrics — disable auto-bidding (phasing out)
- [ ] Google Cloud budget alerts
- [ ] Ads optimizer — switch from --dry-run to --execute (after soak)
- [ ] Bundle offers (Phase 4 — needs product strategy)
- [ ] Quintex follow-up (49 days, dead — archive)
- [ ] OTL / Mitch Hamilton (44 days, dead — archive)
- [ ] Jaan Malik tracking email (overdue)
- [ ] PayPal 2025 1099-K — flag for accountant
- [ ] 2oz Thick Auto budget increase ($25→$50/day — leaving revenue on table)
- [ ] Weekly ads performance cron
- [ ] Oversold inventory reconciliation (blocked by Shopify API)
- [ ] PPC Scaling Planner review — $2K/day roadmap

## 💸 OPS DEBT TRACKER (Q2 Day 1)
| Item | Days | Accrued | Rate | Fix Time |
|------|------|---------|------|----------|
| ShipBob UROs | 53 | ~$954 | $18/day | 15 min |
| Amazon CA | 15 | ~$225 | $15/day | 55 min |
| Teikametrics | 2 | $149 | one-time | 5 min |
| Heather SLA | 17 | seller metrics | compounding | 5 min |
| Shopify API | 39 | inventory blind | — | 15 min |
| Insurance audit | 60 | non-renewal risk | — | 15 min |
| Vercel billing | — | $53.31 | one-time | 2 min |
| **Daily burn** | — | **$33/day** | — | — |
| **Total hard costs** | — | **$1,381+** | — | — |

## 📊 KEY METRICS (as of 3/31)
- **Revenue run rate:** ~$638K (Amazon $531K + Shopify $107K)
- **PPC:** $773/day spend, $1,867 sales, 2.42× ROAS, 41.4% ACoS
- **B2B cash collected Q1:** $12K+ (Garcor $4,806 + R&R $1,290 + Ron Southern + DMS + Donaldson $2,340)
- **B2B outstanding:** $7,452 (A3 $3,312 + Noveon $4,140)
- **Pipeline active:** 5 deals + 8 queued prospects
- **Q1 grade:** C+ (revenue stable, B2B launched, ops debt accumulated)
